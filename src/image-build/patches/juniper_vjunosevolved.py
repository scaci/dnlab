"""Per-kind patch plan for juniper_vjunosevolved (vrnetlab vJunosEvolved).

Applies persist-overlay to ``/vrnetlab.py``. The kind's ``launch.py``
does not duplicate the overlay computation (the base class handles it
entirely), so launch.py does not need patching for persistence.
"""
